...
interface QRDeviceClientProps {
  deviceId: string;
}

export default function QRDeviceClient({ deviceId }: QRDeviceClientProps) {
  ...
  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault();
    // Same credentials as main dashboard
        ...
        {isLoading ? (
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-lg p-6">
              ...
              {deviceData.details.description && (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div
                  className={`p-4 rounded-lg ${
                    deviceData.sensorData.limitSwitchPressed
                      ? "bg-yellow-50"
                      : "bg-green-50"
                  }`}
                >
                  <p className="text-sm text-gray-600">Limit Switch</p>
                  <p
                    className={`font-semibold ${getStatusColor(
                      !deviceData.sensorData.limitSwitchPressed
                    )}`}
                ...
                <div
                  className={`p-4 rounded-lg ${
                    deviceData.sensorData.locationBreach
                      ? "bg-red-50"
                      : "bg-green-50"
                  }`}
                >
                  <p className="text-sm text-gray-600">Location Security</p>
                  <p
                    className={`font-semibold ${getStatusColor(
                      !deviceData.sensorData.locationBreach
                    )}`}
            ...
            <div className="bg-white rounded-xl shadow-lg p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-gray-50 p-4 rounded-lg">
                  <p className="text-sm text-gray-600">Last Wake Reason</p>
                  <p className="font-semibold text-gray-900">
                    {deviceData.sensorData.wakeReason || "Unknown"}
                  </p>
                </div>
                <div className="bg-gray-50 p-4 rounded-lg">
                  <p className="text-sm text-gray-600">Last Update</p>
                  <p className="font-semibold text-gray-900">
                    {formatTimestamp(deviceData.sensorData.timestamp || 0)}
                  </p>
                </div>
                {deviceData.sensorData.altitude !== undefined && (
                  <div className="bg-gray-50 p-4 rounded-lg">
                    <p className="text-sm text-gray-600">Altitude</p>
                    <p className="font-semibold text-gray-900">
                      {deviceData.sensorData.altitude.toFixed(1)}m
                    </p>
                  </div>
                )}
              </div>
